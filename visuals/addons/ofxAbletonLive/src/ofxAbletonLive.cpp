...
void ofxAbletonLive::update()
{
    while(receiver.hasWaitingMessages())
    {
        ...
        if      (m.getAddress() == "/live/device/param") {
			...
			if (getTempo() != tempo) this->tempo = tempo;
		}
		else if (m.getAddress() == "/live/time") {
			float time = m.getArgAsFloat(0);
			if (getTime() != time) this->time = time;
		}
		else if (m.getAddress() == "/live/play") {
			playing = m.getArgAsFloat(0) == 2 ? true : false;
		}
...
void ofxAbletonLive::requestQuantization()
{
	ofxOscMessage msg;
	msg.setAddress("/live/quantization");
	sender.sendMessage(msg);
